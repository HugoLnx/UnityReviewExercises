...
namespace ReviewLinq
{
    public class QuerierSolution
    {
        ...
        {
            return _list.Single(n => n > 4);
        }

        public List<int> MiddleX(int amount)
        {
            return _list.Skip((_list.Count-amount)/2).Take(amount).ToList();
        }

        public List<int> LastX(int amount)
        {
            return _list.TakeLast(amount).ToList();
        }

        public List<int> FirstSequenceOfEvens()
        {
            return _list.SkipWhile(n => n % 2 != 0).TakeWhile(n => n % 2 == 0).ToList();
        }

        public List<int> SortEvenFirstThenDescending()
        {
            return _list.OrderBy(n => n % 2).ThenByDescending(n => n).ToList();
        }
        ...
        {
            return _list.Intersect(otherNumbers).ToList();
        }

        public List<int> UniqueValuesExcludingOnesFrom(List<int> otherNumbers)
        {
            return _list.Except(otherNumbers).ToList();
        }

        public List<int> UniqueValuesIncludingOnesFrom(List<int> otherNumbers)
        {
            return _list.Union(otherNumbers).ToList();
        }

        public Dictionary<int, List<int>> GroupByMod3()
        {
            return _list.GroupBy(n => n % 3).ToDictionary(g => g.Key, g => g.ToList());
        }

        public List<List<int>> SplitByMod3()
        {
            return _list.GroupBy(n => n % 3).OrderBy(g => g.Key).Select(g => g.ToList()).ToList();
        }
        ...
        {
            return _list.Select(n => n % 3).Sum();
        }

        public float AvgAllMod3()
        {
            return _list.Select(n => n % 3f).Where(n => n > 0).Average();
        }
    }
}